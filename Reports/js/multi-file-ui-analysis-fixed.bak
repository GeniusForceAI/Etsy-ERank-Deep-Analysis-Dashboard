...
const MultiFileUIAnalysis = (function() {
    ...
    const elements = {
        ...
        // Charts
        keywordDistChart: document.getElementById('keywordDistributionChart'),
        metricsComparisonChart: document.getElementById('performanceChart'),
        performanceMatrixChart: document.getElementById('performanceMatrixChart'),
    ...
    function createVisualizations() {
        // Only create visualizations if MultiFileVisualizations is available
        if (typeof window.MultiFileVisualizations !== 'object') {
            console.error('MultiFileVisualizations module not loaded');
            return;
        }
        
        try {
            // Create keyword distribution chart if element exists
            if (elements.keywordDistChart) {
                window.MultiFileVisualizations.createKeywordDistributionChart('keywordDistributionChart', {
                    stats: analysisResults.stats,
                    aList: analysisResults.aList
                });
            }
            
            // Create metrics comparison chart if element exists
            if (elements.metricsComparisonChart) {
                // Create metrics comparison chart with the 3 lists
                window.MultiFileVisualizations.createMetricsComparisonChart(
                    'performanceChart',
                    { 
                        aList: analysisResults.aList, 
                        bList: analysisResults.bList, 
                        cList: analysisResults.cList
                    }
                );
            }
            
            // Create performance matrix if MultiFileVisualizations has this method
            if (elements.performanceMatrixChart && typeof window.MultiFileVisualizations.createPerformanceMatrixChart === 'function') {
                // Create performance matrix chart
                window.MultiFileVisualizations.createPerformanceMatrixChart('performanceMatrixChart', {
                    allScored: analysisResults.allScored,
                    aList: analysisResults.aList,
                    bList: analysisResults.bList,
                    cList: analysisResults.cList
                });
            ...
            // Set up x-axis and y-axis change handlers
            if (elements.xAxisMatrix && elements.yAxisMatrix) {
                elements.xAxisMatrix.addEventListener('change', updatePerformanceMatrix);
                elements.yAxisMatrix.addEventListener('change', updatePerformanceMatrix);
            }
